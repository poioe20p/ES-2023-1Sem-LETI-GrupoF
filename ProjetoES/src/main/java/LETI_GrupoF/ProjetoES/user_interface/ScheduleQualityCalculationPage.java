...
//        Panel que corresponde ao separador que contém o display das fórmulas criadas
	...
	private JPanel bottomFormulaCreatingPanel(JFrame previousFrame) {
		...
		JButton removeLastAddedFormula = LayoutDefinable.defineButtonLayout(Color.RED, Color.white, "Remove Formula",
				new Dimension(130, 40));
		removeLastAddedFormula.addActionListener(e -> updateFormulaListPane(-1, null));
		addFormula = LayoutDefinable.defineButtonLayout(Color.GREEN, Color.WHITE, "Add Formula",
				new Dimension(130, 40));
		addFormula.addActionListener(e -> updateFormulaListPane(1, getVariablesFromFormula()));
		addFormula.setEnabled(false);
		calculateScheduleQuality = LayoutDefinable.defineButtonLayout(Color.BLUE, Color.WHITE, "Schedule Quality",
				new Dimension(130, 40));
		JButton goBackButton = LayoutDefinable.defineButtonLayout(Color.RED, Color.white, "Go Back",
				new Dimension(130, 40));
		goBackButton.addActionListener(e -> {
			previousFrame.setVisible(true);
			LayoutDefinable.setVisibility(this, false);
		});
		JButton resetListOptions = LayoutDefinable.defineButtonLayout(Color.BLACK, Color.WHITE, "Reset Options",
				new Dimension(130, 40));
